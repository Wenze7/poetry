...
def get_args():
    ...
    parser.add_argument('--valid', type=bool, default=False, help='测试还是预测')
...
def predict_one_sample(args, model, tokenizer, keyword):
    ...
    for step in range(args.generate_max_len):
        if step == 0:
            outputs = model.forward2(input_ids=input_tensors, output_hidden_states=True)
        else:

            outputs = model.forward2(input_ids=input_tensors[:, -1].unsqueeze(-1),
    ...
    return candidate_responses
        # print((filter_logits != -float("Inf")).sum())
...
if __name__ == '__main__':
    ...
    model = MYGPT2LMHeadModel.from_pretrained("./ModelConfig/model_" + args.mode + "/final_model")
